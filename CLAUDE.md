...
- **Paragraph-aware chunking:** CHUNK_TARGET=1200, CHUNK_MAX=1800, CHUNK_OVERLAP=200. No MIN_CHUNK_LENGTH — merge/split logic handles size without a hard floor. Split on `\n+` (handles both cheerio and Puppeteer whitespace). Sentence-boundary overlap. Paragraph-level hash dedup (seenParaHashes shared across batches) strips boilerplate before chunking. Chunk-level dedup as secondary safety net. **H1/H2/H3 headings are always exempt from dedup** — prefixed with `[H1]`/`[H2]`/`[H3]` markers in both cheerio and Puppeteer paths so page titles (staff names, etc.) are never silently dropped when the same name appears on a listing page first.
- **Multi-URL chunking for thin pages:** Pages with < 300 chars of text (e.g. staff cards with only name/title/email) are never embedded individually — they're buffered during the crawl and grouped by parent URL (e.g. all `/staff/*` pages → one combined chunk attributed to `/staff/`). Each page becomes a "card" with `[Source: url]`, `[H1] name`, and body. Cards are packed into CHUNK_TARGET-sized group chunks. `sourceUrls: [String]` on the Chunk model tracks all contributing page URLs. Group chunks bypass seenParaHashes dedup so thin page content (titles, emails) is never silently stripped. On rescrape, groups are rebuilt only when ≥1 member page changed; old group chunks (keyed by groupUrl) are deleted and replaced atomically.
...
Full prompt: `prompts/leo-system-prompt.md`. Current version: **v2.3**
...
- ✅ Multi-URL chunking — thin pages (< 300 chars) grouped by parent URL into combined group chunks; `sourceUrls: [String]` on Chunk/ArchivedChunk models; fixes staff cards with only name/title/email being silently dropped
...
- ✅ Production deployment — backend on Railway (api.leo-ai.chat), dashboard on Netlify (leo-ai.app). Email switched from Proton Bridge/nodemailer to Resend SDK (email.js service). Dashboard API URL driven by VITE_API_URL env var. Railway service Root Directory is `/` (whole repo in build context so index.js can serve ../widget at /demo); root `railway.toml` forces the NIXPACKS builder (the service's default is Railpack, which ignores nixpacks.toml and can't detect a provider at the root — every deploy failed for ~19h until 91d5a5d, 2026-08-08); root `nixpacks.toml` is the build config (nodejs_20 — nodejs_22 undefined in nixpacks' pinned nixpkgs; yarn install runs --ignore-engines because nix yarn runs under its own internal node 18; system Chromium + --no-sandbox for Puppeteer). backend/nixpacks.toml is now inert and pending removal.
...
- ✅ Scraper — `keepPara(p, cs)` unified paragraph filter: `PHONE_RE`, `MONEY_RE`, `TIME_RE`, `ADDRESS_FRAGMENT_RE` always-on; `SOCIAL_HANDLE_RE` and `SHORT_URL_RE` gated on entity crawlSettings flags.
...
- ✅ Scraper — Variant price sweep for shop product pages: Puppeteer iterates `<select>` options, dispatches change/input events, polls for live price per variant. Global range-anchor fast path (deepest element matching PRICE_RANGE_RE); falls back to outward DOM search per-select if anchor doesn't update. Sweep triggered by `knownHasVariants`, detected price range, or absent single price.
- ✅ ScrapedPage — `hasVariants: Boolean` field; set true when sweep finds variant prices; future rescrapes skip trigger detection and always run sweep on tagged pages.
- ✅ Entity crawlSettings — `keepSocialHandles`, `keepShortUrls`, `variantPriceSweep` Boolean toggles; Settings UI card; threaded through scraper and leoRefresh.
...
- ✅ Scraper — semantic block element floor: `p`, `td`, `th`, `dt`, `dd`, `blockquote`, `figcaption`, `address` content tagged with `[SB]` prefix in `extractStructuredText`; `keepPara` uses 4-char floor for `[SB]` paragraphs (vs 20-char for generic divs/inline elements); `stripBlockMarker` strips prefix after filtering everywhere text is consumed. Preserves "Closed Sundays", staff names, table cell values that were previously silently dropped.
- ✅ Scraper — Pass 1 Haiku structural analysis: home page fetched before main crawl loop; compact DOM summary (tag + class/id + 50-char text, 3-level depth) sent to `analyzePageStructure()` in claude.js; returned CSS selectors applied as cheerio pre-filter (and Puppeteer DOM remove) on every subsequent page. Targets cookie banners, promo ribbons, social share widgets. Fails gracefully — any error skips filters and continues scrape. ~$0.0003–0.0005 per scrape.
- ✅ Admin Console Log Viewer — `consoleBuf.js` monkey-patches `console.log/warn/error` at startup; in-memory circular buffer (last 500 entries) emits `log_entry` socket events to the `superadmin` room. `GET /api/admin/logs` returns buffer history. Dashboard `/logs` page is a live streaming terminal: history on mount, streams new entries via socket, auto-scroll with pause/resume (also pauses on manual scroll-up), filter by All/Warn+/Errors, color-coded by level, monospace. Superadmin-only.
- ✅ Admin MCP server — `mcp/` directory; `@modelcontextprotocol/sdk` + stdio transport; 8 superadmin tools: `send_test_message` (full debug output: reply, model, classifierRoute/Reason, topScore, hadContext), `search_chunks` (raw scored RAG hits), `kick_off_scrape`, `kick_off_rescrape`, `list_entities`, `get_entity`, `get_quota_usage`, `get_recent_logs`. `.mcp.json` at root for Claude Code auto-discovery. Backend: `GET /api/admin/search` (superadmin-gated Atlas Vector Search returning scored chunks) added to support `search_chunks`.
- ✅ API key system — `ApiKey` model (keyHash SHA-256, label, scope, lastUsedAt); `POST/GET/DELETE /api/admin/api-keys` superadmin endpoints; `requireAdminAuth` middleware accepts either JWT superadmin or `X-API-Key` header on all admin routes. Keys can't self-manage (prevents escalation). MCP server updated to use `X-API-Key` header with `API_KEY` env var. Dashboard Admin → API Keys page: generate with label, one-time reveal modal with copy button, list with revoke. `mcp/.env.example` and `mcp/README.md` updated.
- ✅ Overview Usage Panel — `UsagePanel.vue` replaces raw "Total Messages" card; plan-aware: Free shows X/100 progress bar with green/yellow/red burn-rate projection + reset date + "Not enough data yet" guard; PAYG shows X/next-milestone, projected monthly cost, retroactive-savings callout when on track to cross a threshold; Infinity/Lifetime shows X/∞ with no projection. `billingPeriodStart` field added to Entity model (set in chat.js on first message and on monthly reset) for accurate days-elapsed calculation. Reusable component in `dashboard/src/components/`.
